...
const LoginInput: FC<IProps> = (props) => {
  ...
  return (
    <div className="w-80 flex relative flex-col items-center">
      ...
      <input
        {...field}
        {...props}
        className={`outline-none border border-solid border-bg__third bg-primary w-full h-12 text-base rounded px-3 mb-3 text-color__primary focus:border-blue__color ${
          meta.touched && meta.error && "border-[#e84118]"
        } ${props.className}`}
      />
      {meta.touched && meta.error && (
        <i className={`error_icon absolute right-1 ${props.bottom?"top-[calc(50%-44px)]" :"top-[calc(50%+12px)]"}  scale-[0.8] md:top-1/2 md:-translate-y-[calc(50%+6px)]`}  />
      )}
      {meta.touched && meta.error && props.bottom && (
